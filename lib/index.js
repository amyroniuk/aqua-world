...
var updateXML = function updateXML(path, prices) {
  return new Promise(function (resolve, reject) {
    ...
    (0, _xml2js.parseString)(xml_input, function (err, result) {
      ...
      console.info("Total offers count: ".concat(result.offers.offer.length));
      var updatedOffersCount = 0;
      result.offers.offer.forEach(function (offer) {
        var next_offer = {};
        var offerId = offer.$.id;
        var price = prices[offerId];

        if (price === undefined) {
          console.warn("Price for offer #".concat(offerId, " was not found in CSV"));
        } else {
          updatedOffersCount++;
        }
      ...
      console.info("Updated offers count: ".concat(updatedOffersCount));
...
readCSV(csvPath).then(function (rows) {
  ...
  console.info("Saved output to ".concat(xmlOutPath));
});